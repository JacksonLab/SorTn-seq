...
# check similarity between samples using the biological coefficient of variation (top n features is set by bcv.features)
    ...
    ggplot(mds.plot) + geom_point(aes(x,y,color=sample.type)) +
                ...
                scale_x_continuous(expand=c(0,0),limits=c(floor(min(mds.plot$x)*5)/5,ceiling(max(mds.plot$x)*5)/5)) +
                scale_y_continuous(expand=c(0,0),limits=c(floor(min(mds.plot$y)*5)/5,ceiling(max(mds.plot$y)*5)/5)) +
                theme_light() +
...
# volcano plots # Padj < 0.1 and log FC > 0
  ...
  volcano_plot_gg<-function(plot.sample,point.colour){
    ...
    print(ggplot(all.results.long%>%filter(sample==plot.sample),aes(logFC,-log10(p.adj))) +
      geom_point(aes(col=p.adj<0.1 & logFC >0),show.legend=F,size=0.5) +
      scale_color_manual(values=c("grey 35",point.colour)) +
      geom_hline(yintercept=1,size=0.5) +
      geom_vline(xintercept=0,size=0.5) +
      # use the same scale limits for all plots
      scale_x_continuous(expand=c(0,0),limits=c(floor(min(all.results.long$logFC)),ceiling(max(all.results.long$logFC)))) +
      scale_y_continuous(expand=c(0,0),limits=c(0,ceiling(-log10(min(all.results.long$p.adj))))) +  
      theme_light()+
...
enrichment<-all.results.long%>%filter(sample %in% c("high","low") &
                                    ...
                                    mutate(color=ifelse(p.adj<=0.1,color,"depleted"))
...
plot<-ggplot(to.plot,aes(x=enrichment,y=-log10(p.adj),color=color,size=ins.per.kb.enriched)) +
      ...
      scale_color_manual(values=c("grey 35","red","dodgerblue3"),guide=F) +
      geom_hline(yintercept=1,size=0.5) +
      geom_vline(xintercept=0,size=0.5) +
      # use the same scale limits for all plots
      scale_x_continuous(expand=c(0,0),limits=c(-ceiling(max(to.plot$logFC)),ceiling(max(to.plot$logFC)))) +
      scale_y_continuous(expand=c(0,0),limits=c(0,ceiling(-log10(min(to.plot$p.adj))/5)*5)) +  
      theme_light()+
